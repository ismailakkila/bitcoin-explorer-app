...
BLOCK_DIFFICULTY_ADJUSTMENT_PERIOD = 2016
BLOCK_HALVING_PERIOD = 210000
...
class Block_Full(Block):
    def __init__(self, version, prev_block, merkle_root,
                 timestamp, bits, nonce, txs):
        self.version = version
        self.prev_block = prev_block
        self.merkle_root = merkle_root
        self.timestamp = timestamp
    ...
    def expected_block_reward(self, block_height):
        block_halving_period = block_height // BLOCK_HALVING_PERIOD
        expected_block_reward = GENESIS_BLOCK_REWARD
        for i in range(block_halving_period):
            expected_block_reward //=  2
            if expected_block_reward == 0:
                break
        return expected_block_reward

    def block_reward(self, total_fees):
        coinbase_tx = self.txs[0]
        total_payout = 0
        for tx_out in coinbase_tx.tx_outs:
            total_payout += tx_out.amount
        return total_payout - total_fees
