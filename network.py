...
import sys
import shutil
from errors import TorNotAvailable
...
BIND_SOCKS_PORT_ERROR_MSG = 'Process terminated: Failed to bind one of the listener ports.'
...
class BlockMessage:
    ...
    def __init__(self, version, prev_block, merkle_root, timestamp, bits, nonce, txs):
        self.version = version
...
class SimpleNode:
    ...
    def __init__(self, host, port=None, testnet=False, logging=False, tor=False, process_name="main"):
        ...
        if tor:
            self.tor = False
            socks_port = randint(9000, 9999)

            if sys.platform == "darwin" or sys.platform == "linux":
                tor_executable_dir_path = os.path.join(str(Path.home()), ".bitcoin-explorer-app", "tor")
            elif sys.platform == "win32":
                tor_executable_dir_path = os.path.join(str(Path.home()), "bitcoin-explorer-app", "tor.exe")
            else:
                tor_executable_dir_path = os.path.join(str(Path.home()), "bitcoin-explorer-app", "tor")
            if os.path.exists(tor_executable_dir_path):
                tor_cmd = tor_executable_dir_path
            else:
                tor_cmd = "tor"

            tor_data_dir_path = os.path.join(check_folder(is_dist_mode()), "tor_" + process_name)
            if not os.path.exists(tor_data_dir_path):
                os.mkdir(tor_data_dir_path, 0o744)
            else:
                shutil.rmtree(tor_data_dir_path)
            while not self.tor:
                try:
                    if logging:
                        tor_process = stem.process.launch_tor_with_config(
                            config = {
                                ...
                                "DataDirectory": tor_data_dir_path
                            },
                            tor_cmd=tor_cmd,
                            init_msg_handler=logging,
                            take_ownership=True,
                            timeout=60
                        )
                    else:
                    ...
                    self.tor = tor_process
                    self.socks_port = socks_port
                except OSError as os_error:
                    if BIND_SOCKS_PORT_ERROR_MSG == os_error.__str__():
                        pass
                    else:
        ...
        if tor:
            s = socks.socksocket()
            s.set_proxy(socks.SOCKS5, "localhost", socks_port)
            self.socket = s
        else:
