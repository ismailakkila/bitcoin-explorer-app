...
class Script:
    ...
    def raw_serialize(self, coinbase_tx=False):
        # initialize what we'll send back
        result = b''
        if coinbase_tx and len(self.cmds) == 1 and type(self.cmds[0]) == bytes:
            result = self.cmds[0]
            return result
        # go through each cmd
        for cmd in self.cmds:
    ...
    def serialize(self, coinbase_tx=False):
        ...
        result = self.raw_serialize(coinbase_tx=coinbase_tx)
    ...
    def is_p2pk_script_pubkey(self):
        return len(self.cmds) == 2 and type(self.cmds[0]) == bytes and \
            (len(self.cmds[0]) == 65 or len(self.cmds[0]) == 33) and self.cmds[1] == 0xac
