...
class Tx:
    ...
    def __init__(self, version, tx_ins, tx_outs,
        locktime, coinbase_tx=False, testnet=False, segwit=False):
    ...
    # tag::source2[]
    @classmethod
    def parse(cls, s, coinbase_tx=False, testnet=False):
        s.read(4)  # <1>
        if s.read(1) == b'\x00':  # <2>
        ...
        s.seek(-5, 1)  # <3>
        return parse_method(s, coinbase_tx=coinbase_tx, testnet=testnet)

    @classmethod
    def parse_legacy(cls, s, coinbase_tx=False, testnet=False):
        version = little_endian_to_int(s.read(4))   # <4>
        num_inputs = read_varint(s)
        inputs = []
        for _ in range(num_inputs):
            inputs.append(TxIn.parse(s, coinbase_tx=coinbase_tx))
        num_outputs = read_varint(s)
        outputs = []
        for _ in range(num_outputs):
            outputs.append(TxOut.parse(s))
        locktime = little_endian_to_int(s.read(4))
        return cls(version, inputs, outputs, locktime, coinbase_tx=coinbase_tx,
                   testnet=testnet, segwit=False)
    # end::source2[]

    # tag::source3[]
    @classmethod
    def parse_segwit(cls, s, coinbase_tx=False, testnet=False):
    ...
    def serialize_legacy(self):  # <1>
        ...
        for tx_in in self.tx_ins:
            result += tx_in.serialize(coinbase_tx=self.coinbase_tx)
    ...
    def size(self):
        if self.segwit:
            return len(self.serialize_segwit())
        else:
            return len(self.serialize_legacy())
...
class TxIn:
    ...
    @classmethod
    def parse(cls, s, coinbase_tx=False):
        ...
        script_sig = Script.parse(s, coinbase_tx=coinbase_tx)
        # sequence is an integer in 4 bytes, little-endian
        sequence = little_endian_to_int(s.read(4))
        # return an instance of the class (see __init__ for args)
        return cls(prev_tx, prev_index, script_sig, sequence)

    def serialize(self, coinbase_tx=False):
        ...
        result += self.script_sig.serialize(coinbase_tx=coinbase_tx)
